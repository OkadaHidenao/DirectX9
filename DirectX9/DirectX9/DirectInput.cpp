...
HRESULT DirectInput::InitMouse()
{
	...
	//キーボードのデバイス作成と同じ要領
	//作るデバイスのタイプが違うだけ
	hresult = pDInput->CreateDevice(GUID_SysMouse,
		&pMouseDevice, NULL);
	...
	POINT localPos;
	//localとしたのはクライアント領域の左上角を基準にした座標だから
...
HRESULT DirectInput::Update()
{
	...
	{
		//前回のマウス状態を丸ごとコピー
		memcpy(&mouseInputPreview, 
			&mouseInput,
			sizeof(DIMOUSESTATE2));

		pMouseDevice->
			GetDeviceState(
				sizeof(DIMOUSESTATE2), &mouseInput);
		...
		for (int i = 0; i < MOUSE_BUTTON_MAX; i++)
		{
			...
			if (mouseInput.rgbButtons[i])
			{
				...
				{
					//押されていない場合はフレーム数をリセット
					mouseButtonCounter[i] = 0;
				}
			}
		}
	}
	//マウス座標の取得関数はDirectInputのコードを使用していない
	//座標の取り扱いがこちらのほうがわかりやすかった
...
bool DirectInput::MouseButtonJustPressed(int num)
{
	if (num >= 0 && num < MOUSE_BUTTON_MAX)
	{
		...
		return (mouseInput.rgbButtons[num] >= 1) &&
			(mouseInputPreview.rgbButtons[num] == 0);
	}
	else
	{
		return false;//配列範囲外
...
bool DirectInput::MouseButtonJustReleased(int num)
{
	if (num >= 0 && num < MOUSE_BUTTON_MAX)
	{
		//現フレームで押されていない　かつ　前フレームで押されている
		return (mouseInput.rgbButtons[num] ==0 ) &&
			(mouseInputPreview.rgbButtons[num] >=1);
	}
	else
	{
		return false;//配列範囲外
